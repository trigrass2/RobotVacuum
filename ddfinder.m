...
elseif crel(1)<0 && crel(2)>=0 %Quadrant= 3
    % If robot in quadrant 3, choose a destination in quadrant 1
    for qL=1:c(1)
...
elseif crel(1)<0 && crel(2)<0  %Quadrand=2
    % If robot in quadrant 2, choose a destination in quadrant 4
    ...
    % If robot in quadrant 1, choose a destination in quadrant 3
    for qL=size(R,1):-1:c(1)
        for qW=1:c(2)
            if R(qL,qW)~=0 && R(qL,qW)~=5 && R(qL,qW)~=1 && R(qL,qW)~=2
                dest=[qL,qW];
